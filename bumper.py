...
# Function to check various items
def check_site():
	...
	if soup.find('div', {'class': 'g-recaptcha'}):
		log.error("CAPTCHA found.")
		return False
	elif not soup.find('img', {'src': 'logo.php'}):
		log.error("OGUsers is currently not functional [1].")
		return False
	elif response.status_code != 200:
		log.error("OGUsers is currently not functional [2].")
		return False
	else:
...
def bump(data):
	try:
		...
		print(e)
		log.error("There has been an error whilst bumping.")
